...
#include "UI/Machines/Triage/EMRCommonTriageMainWidget.h"
...
namespace
{
	bool ShouldDisplayPatientInReceptionList(const AEMRPatient* Patient)
	{
		if (!Patient)
		{
			return false;
...
void UEMRCommonTriageMainWidget::NativeConstruct()
{
	...
	if (CommonButton_Exit)
	{
		CommonButton_Exit->OnClicked().RemoveAll(this);
		CommonButton_Exit->OnClicked().AddUObject(this, &ThisClass::HandleExitClicked);
	}
...
void UEMRCommonTriageMainWidget::NativeDestruct()
{
	ClearPatientCards();
...
void UEMRCommonTriageMainWidget::NativeOnActivated()
{
	Super::NativeOnActivated();

	if (UWidget* PreferredWidget = GetPreferredFocusWidget_Implementation())
	{
		PreferredWidget->SetFocus();
	}
}

void UEMRCommonTriageMainWidget::NativeOnDeactivated()
{
...
UWidget* UEMRCommonTriageMainWidget::NativeGetDesiredFocusTarget() const
{
	return GetPreferredFocusWidget_Implementation();
}

UWidget* UEMRCommonTriageMainWidget::GetPreferredFocusWidget_Implementation() const
{
	if (!CommonTileView_PatientCards)
	{
		return nullptr;
...
void UEMRCommonTriageMainWidget::RefreshPatientCards()
{
	ClearPatientCards();
	...
	for (const FEMRWaitingPatientEntry& Entry : WaitingPatients)
	{
		...
		if (!ShouldDisplayPatientInReceptionList(Patient))
		{
			continue;
		}
...
void UEMRCommonTriageMainWidget::ClearPatientCards()
{
...
UEMRWaitingRoomManagerComponent* UEMRCommonTriageMainWidget::ResolveWaitingRoomManager()
{
	if (WaitingRoomManager.IsValid())
	{
...
void UEMRCommonTriageMainWidget::OnSeatAssigned(UEMRWaitingSeatComponent* Seat, AActor* PatientActor)
{
	RefreshPatientCards();
}

void UEMRCommonTriageMainWidget::OnSeatReleased(UEMRWaitingSeatComponent* Seat)
{
	RefreshPatientCards();
}

void UEMRCommonTriageMainWidget::OnPatientCalled(AActor* PatientActor)
{
	RefreshPatientCards();
...
void UEMRCommonTriageMainWidget::HandleWaitingPatientsChanged()
{
	RefreshPatientCards();
}

void UEMRCommonTriageMainWidget::HandlePatientPhaseChanged(FGameplayTag NewPhase)
{
	RefreshPatientCards();
}

void UEMRCommonTriageMainWidget::HandleEntryWidgetReleased(UUserWidget& Widget)
{
...
void UEMRCommonTriageMainWidget::HandleExitClicked()
{
	OnTriageExitRequested.Broadcast();
}

void UEMRCommonTriageMainWidget::HandlePatientClicked(UObject* Item)
{
...
void UEMRCommonTriageMainWidget::HandlePanelCloseRequested()
{
	if (CachedPanelWidget.IsValid())
	{
...
AEMRReceptionMachine* UEMRCommonTriageMainWidget::ResolveReceptionMachine()
{
