...
FString UUnrealMCPBridge::ExecuteCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    auto SerializeResponse = [](const TSharedPtr<FJsonObject>& InResponse) -> FString
    {
        FString ResultString;
        TSharedRef<TJsonWriter<>> Writer = TJsonWriterFactory<>::Create(&ResultString);
        FJsonSerializer::Serialize(InResponse.ToSharedRef(), Writer);
        return ResultString;
    };

    auto BuildResponse = [this, &CommandType, &Params]() -> TSharedPtr<FJsonObject>
    {
        ...
        try
        {
            ...
            // Editor Commands (including actor manipulation)
                     ...
                     CommandType == TEXT("get_actor_full_info") ||
                     CommandType == TEXT("set_actor_property") ||
                     CommandType == TEXT("set_actor_component_property") ||
                     CommandType == TEXT("fix_invalid_nanite_materials") ||
                     CommandType == TEXT("open_level") ||
                     CommandType == TEXT("spawn_blueprint_actor") ||
                     CommandType == TEXT("focus_viewport") ||
                     CommandType == TEXT("capture_editor_screenshot") ||
    ...
    // Loading maps while still inside the current task graph tick can trigger teardown assertions.
    // Defer open_level to the next core ticker tick so world cleanup happens in a safer phase.
    if (CommandType == TEXT("open_level"))
    {
        AsyncTask(ENamedThreads::GameThread, [BuildResponse, SerializeResponse, Promise]()
        {
            FTSTicker::GetCoreTicker().AddTicker(
                FTickerDelegate::CreateLambda([BuildResponse, SerializeResponse, Promise](float)
                {
                    const TSharedPtr<FJsonObject> ResponseJson = BuildResponse();
                    Promise->SetValue(SerializeResponse(ResponseJson));
                    return false;
                }),
                0.0f
            );
