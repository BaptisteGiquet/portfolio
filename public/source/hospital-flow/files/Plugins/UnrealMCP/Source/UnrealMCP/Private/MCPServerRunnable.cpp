...
// Buffer size for receiving data (leave room for null terminator)
    ...
    while (bRunning)
    {
        ...
        if (ListenerSocket->HasPendingConnection(bPending) && bPending)
        {
            ...
            if (ClientSocket.IsValid())
            {
                ...
                uint8 Buffer[RecvBufferSize + 1];
                while (bRunning)
                {
                    int32 BytesRead = 0;
                    if (ClientSocket->Recv(Buffer, RecvBufferSize, BytesRead))
                    {
                        if (BytesRead == 0)
                        {
                        ...
                        if (FJsonSerializer::Deserialize(Reader, JsonObject))
                        {
                            ...
                            if (JsonObject->TryGetStringField(TEXT("type"), CommandType))
                            {
                                ...
                                const uint8* ResponseBytes = reinterpret_cast<const uint8*>(Utf8Response.Get());
                                const int32 ResponseByteLen = Utf8Response.Length();

                                if (!ClientSocket->Send(ResponseBytes, ResponseByteLen, BytesSent))
                                {
...
void FMCPServerRunnable::ProcessMessage(TSharedPtr<FSocket> Client, const FString& Message)
{
    ...
    if (!Client->Send(ResponseBytes, ResponseByteLen, BytesSent))
    {
