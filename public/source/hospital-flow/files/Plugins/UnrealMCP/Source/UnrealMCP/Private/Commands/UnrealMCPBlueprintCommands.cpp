...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleSetComponentProperty(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Get the component template from SCS node first, then fallback to CDO components
    ...
    else if (Blueprint->GeneratedClass)
    {
        ...
        if (DefaultActor)
        {
            TInlineComponentArray<UActorComponent*> CDOComponents(DefaultActor);
            for (UActorComponent* CDOComponent : CDOComponents)
            {
                if (!CDOComponent)
                {
                    continue;
                ...
                const FString CDOComponentName = CDOComponent->GetName();
                const bool bNameMatch =
                    CDOComponentName.Equals(ComponentName, ESearchCase::IgnoreCase) ||
                    CDOComponentName.Contains(ComponentName, ESearchCase::IgnoreCase) ||
                    ComponentName.Contains(CDOComponentName, ESearchCase::IgnoreCase);

                if (bNameMatch)
                {
                    ComponentTemplate = CDOComponent;
                    UE_LOG(LogTemp, Log, TEXT("SetComponentProperty - Component resolved from CDO: requested '%s', matched '%s' (Class: %s)"),
                        *ComponentName, *CDOComponentName, *ComponentTemplate->GetClass()->GetName());
                    break;
                }
            }
        }
